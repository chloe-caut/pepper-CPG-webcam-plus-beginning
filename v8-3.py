...
def control_robot_hand(motion_service, side, wrist_landmark_time1, wrist_landmark_time2):
    try:
        # Calculate the angle for the movement
        angle = calculate_angle(wrist_landmark_time1, wrist_landmark_time2)
        
        # Ensure side is either 'R' or 'L'
        if side not in ['R', 'L']:
            raise ValueError("Side must be 'R' for right or 'L' for left.")
